...
async def random_hanime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        ydl_opts = {
            ...
            'downloader': 'ffmpeg',  # Use FFmpeg for M3U8 downloads
            'http_headers': {  # Enhanced headers to mimic 1DM+
        ...
        if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
            ...
            if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
                raise Exception("Failed to download video after all attempts. CDN may be blocking or headers need adjustment.")
