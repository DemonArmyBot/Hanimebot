...
async def random_hanime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Step 3: Try yt-dlp if Playwright fails or no M3U8 URLs
        ydl_opts = {
            'quiet': False,
            'no_warnings': False,
            'extract_flat': False,
        ...
        if direct_url == video_url:
            logger.info("Playwright failed or found no M3U8 URLs; trying yt-dlp")
            max_retries = 3
        ...
        # Step 4: Static scraping if both Playwright and yt-dlp fail
        if direct_url == video_url:
