...
async def random_hanime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        ydl_opts = {
            ...
            'ffmpeg_location': '/usr/bin/ffmpeg',  # Path to FFmpeg (apt-installed)
        }
        ...
        try:
            ...
            logger.error(f"yt-dlp extraction error: {ydl_error}")
        ...
        try:
            ...
            logger.error(f"yt-dlp download error: {dl_error}")

        # Check if file was downloaded successfully
        if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
            logger.error("Video download failed; file is empty or not created.")
            # Fallback: Try scraping for iframe or video tags
            ...
            if iframe:
                ...
                try:
                    ...
                    logger.error(f"Embed download error: {embed_error}")
            else:
                # Try finding <video> tags
                ...
                if video_tag and video_tag.find('source', src=True):
                    ...
                    try:
                        ...
                        logger.error(f"Video tag download error: {video_error}")
