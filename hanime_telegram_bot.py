...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_message = (
        ...
        "Optional: Place cookies.txt in /app for authenticated videos.\n"
...
async def random_hanime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        auth_message = "\n⚠️ If the video requires login, add cookies.txt to /app and try again."
        ...
        ydl_opts = {
            ...
            'outtmpl': '%(id)s.%(ext)s',  # Template for downloaded file
            'noplaylist': True,  # Avoid playlists
            'merge_output_format': 'mp4',  # Convert M3U8 to MP4
            'ffmpeg_location': '/usr/bin/ffmpeg',  # Path to FFmpeg
        }
        # Add cookiefile only if cookies.txt exists
        ...
        if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
            ...
            # Retry M3U8 URL directly
            if 'm3u8' in direct_url:
                logger.info(f"Retrying M3U8 URL: {direct_url}")
                try:
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ydl.download([direct_url])
            ...
            if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
                raise Exception("Failed to download video after all attempts. Try adding cookies.txt if authentication is required.")
        ...
        if file_size < 2 * 1024 * 1024 * 1024:  # Less than 2GB
            ...
            await context.bot.send_video(
                ...
                caption=message[:200],  # Telegram caption limit
        ...
        if thumb:
            await context.bot.send_photo(chat_id=CHAT_ID, photo=thumb, caption=message + auth_message)
        else:
            await context.bot.send_message(chat_id=CHAT_ID, text=message + auth_message)
        await update.message.reply_text(f"Oops, video download failed: {str(e)}. Sent link instead.{auth_message}")
