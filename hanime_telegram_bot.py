...
async def random_hanime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Fallback: Scrape for M3U8 URL if extraction fails
        if not direct_url or direct_url == video_url:
            logger.info("Falling back to scraping for M3U8 URL")
            ...
            if m3u8_urls:
                ...
                logger.warning("No M3U8 URLs found in page")
        ...
        if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
            ...
            if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
                raise Exception("Failed to download video after all attempts. Check hanime.tv plugin or CDN access.")
...
def main() -> None:
    ...
    try:
        ...
        return
    try:
        # Reset webhook to clear any conflicts
        import asyncio
