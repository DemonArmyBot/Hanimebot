...
async def reset_webhook():
    ...
    try:
        ...
        logger.info("Webhook reset successfully: %s", response.json())
    ...
    try:
        async with async_playwright() as p:
            ...
            if os.path.exists('cookies.txt'):
                ...
                logger.info("Loaded cookies into Playwright")
        ...
        logger.error(f"Playwright initialization error: {str(e)}")
...
async def random_hanime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        response.raise_for_status()
        logger.info(f"Random page response: {response.status_code}")
        ...
        # Step 2: Try Playwright scraping
        ...
        # Step 3: Static scraping if Playwright fails
        if direct_url == video_url:
            logger.info("Playwright failed or found no M3U8 URLs; trying static scraping")
            ...
            if not m3u8_urls:
                ...
                for script in scripts:
                    if script.string and 'm3u8' in script.string:
                        ...
                        urls = re.findall(r'https?://[^\s]*\.m3u8', script.string)
                        m3u8_urls.extend(urls)
                # Check for iframe or video sources
                iframe = soup.find('iframe', src=True)
                if iframe:
                    m3u8_urls.append(iframe['src'])
                video_tag = soup.find('video')
        ...
        if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
            ...
            if not os.path.exists(temp_file) or os.path.getsize(temp_file) == 0:
                raise Exception("Failed to download video after all attempts. Check hanime.tv access.")
